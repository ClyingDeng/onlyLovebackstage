                        ...
                        if (err) {
                            ...
                            if (results[0].member_grade == 0) {
                                ...
                                if (results[0].member_status == 1) {
                                    ...
                                    if (day >= 0 && day <= 10) {
                                        grade = 1
                                        shopDAO.upmembergrade(userId, grade, function (err, results2) {
                                            if (err) {
                                                res.json({ code: 500, msg: '修改信息失败！', err: err })
                                            } else {
                                                shopDAO.getShopMember(userId, function (err, results3) {
                                                    if (err) {
                                                        res.json({ code: 500, msg: '查询会员信息失败！', err: err })
                                                    } else {
                                                        res.json({ code: 200, data: results3, msg: '查询会员信息个人成功！' })
                                                    }
                                                })
                                            }
                                        })
                        ...
                        if ((results2[0].prop_price) * (user.number) > results2[0].integral) {
                            ...
                            shopDAO.updateintegral(user, integral, function (err, results4) {
                                if (err) {
                                    ...
                                    shopDAO.insertShopProps(user, function (err, results5) {
                                        if (err) {
                                            res.json({ code: 500, msg: '插入数据失败！', err: err })
                                        } else {
